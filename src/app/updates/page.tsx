    ...
    <main className="min-h-screen bg-transparent">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-6 sm:px-6 lg:px-8">
        <section className="rounded-[36px] border border-slate-200 bg-white p-6 shadow-[0_20px_48px_rgba(15,23,42,0.06)] sm:p-7">
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">{"30분마다 남는 수집 실행 기록"}</h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
            {"매시 00분과 30분마다 실행된 공지 수집 결과를 시간순으로 모아둔 페이지입니다. 새 공지가 없어도 실행 기록이 남아서, 어느 시점에 몇 건을 확인했고 실제 새 공지가 있었는지 한눈에 볼 수 있습니다."}
          </p>
        </section>
        ...
        {history.length === 0 ? (
          <section className="rounded-[36px] border border-dashed border-slate-300 bg-[#FBFCFD] px-6 py-16 text-center text-sm text-slate-500">
            {"아직 저장된 수집 기록이 없습니다. 다음 수집 주기부터 차곡차곡 쌓입니다."}
          </section>
        ) : (
                ...
                <section key={snapshot.checkedAt} className="rounded-[36px] border border-slate-200 bg-white p-5 shadow-[0_20px_48px_rgba(15,23,42,0.06)] sm:p-6">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-full bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700">{`새 공지 ${newNoticeCount}건`}</span>
                      <span className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-600">{`전체 수집 ${totalNoticeCount}건`}</span>
                    </div>
                  </div>
