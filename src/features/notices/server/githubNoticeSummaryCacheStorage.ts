...
const FILE_PATH = "data/notice-summary-cache.json";
...
async function fetchGitHubFile() {
  ...
  if (!response.ok) {
    throw new Error(`GitHub summary cache read failed (${response.status})`);
  }

  return (await response.json()) as GitHubContentResponse;
}

export async function loadGitHubNoticeSummaryCacheMap<T>() {
...
export async function saveGitHubNoticeSummaryCacheMap(value: unknown) {
  if (!isGitHubJsonStorageEnabled()) {
    return false;
  }
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      message: "chore: update notice summary cache",
      content,
      branch,
  ...
  if (!response.ok) {
    throw new Error(`GitHub summary cache write failed (${response.status})`);
  }
