...
const FILE_PATH = "data/notice-state.json";
...
async function fetchGitHubFile() {
  ...
  if (!response.ok) {
    throw new Error(`GitHub notice state read failed (${response.status})`);
  }

  return (await response.json()) as GitHubContentResponse;
}

export async function loadGitHubNoticeStateMap<T>() {
...
export async function saveGitHubNoticeStateMap(value: unknown) {
  if (!isGitHubJsonStorageEnabled()) {
    return false;
  }
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      message: "chore: update notice state",
      content,
      branch,
  ...
  if (!response.ok) {
    throw new Error(`GitHub notice state write failed (${response.status})`);
  }
