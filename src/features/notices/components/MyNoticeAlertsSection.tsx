...
function AlertsSnapshotHeader({ fetchedAt, totalCount }: { fetchedAt: string | null; totalCount: number }) {
  return (
    <AppPanel className="p-4 sm:rounded-[36px] sm:p-6">
...
export function MyNoticeAlertsSection({
  ...
  const content = (
    <>
          ...
          <div className="flex flex-wrap gap-2 text-sm text-slate-500">
            ...
            {preferencesUpdatedAt ? (
              <span className="rounded-full bg-slate-100 px-4 py-2 font-semibold text-slate-600">
                {`${TEXT.settingsSaved} ${formatFetchedAt(preferencesUpdatedAt)}`}
              </span>
            ) : null}
          </div>
        </AppHeroSection>
      ...
      {error ? (
        <section className="rounded-[28px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">{error}</section>
      ) : null}

      {!hasPreferences && !isLoading ? (
        <AppEmptyState>{TEXT.noPreferences}</AppEmptyState>
      ) : isLoading ? (
        <AppEmptyState>{TEXT.loading}</AppEmptyState>
      ) : notices.length === 0 ? (
        ...
        <div className="grid min-w-0 gap-4">
          {notices.map((notice) => (
            <article
              ...
              className="min-w-0 rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)] sm:p-5"
            >
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                <span className="max-w-full rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-500 ring-1 ring-slate-200 [overflow-wrap:anywhere]">
                  {notice.category}
                </span>
